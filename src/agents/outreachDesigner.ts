...
- hook: a SHORT observation CLAUSE (aim for one brief phrase of ~70 characters including spaces, NO trailing period) that names something concrete about the company, grounded ONLY in the input or a real Exa/WebFetch result — no fabricated facts. It is inserted as the start of the sentence "<hook> caught my attention...", so it must read grammatically as the subject of that sentence. Good: "Your work on autonomous coding agents" / "The way you're rethinking LLM evals". Bad: a full sentence, or anything ending in a period. Do NOT count characters or run any shell command to check the length — a hard cap is applied automatically in code, so just write a naturally short phrase (~70 chars) and move on.
- companyUrl: the company's official website/homepage URL. Prefer their real homepage if it appears in the input or a real Exa/WebFetch result; otherwise fall back to the funding-article URL from the input. Return exactly "not_found" only if you have no real URL at all — never fabricate a domain.
...
Research briefly with Exa/WebFetch only if needed to ground the hook or the HQ location. NEVER pause to ask the user — decide yourself. Be concrete; no generic filler.

STOP CONDITION — the moment you have the founder name (or "there"), a hook, a URL, and the HQ location/timezone, IMMEDIATELY output the final JSON and end your turn. Do NOT run shell commands, do NOT "finalize" or "sanity-check" with no-op commands, and do NOT keep searching for confirmation you already have. At most a handful of research calls are ever needed; anything grounded in the input requires ZERO tool calls — just return the JSON.${jsonOnly(
